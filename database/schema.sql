...
--
-- WARNING: this is a full RESET script. It DROPs and recreates every table,
-- so never run it against a database with real data. Use it only on a fresh
-- Supabase project, or in the isolated `test` schema (CI does this
-- automatically). Incremental changes to a live database belong in
-- database/migrations/ instead.
--
-- Originally ported from MySQL (database/basud_db.sql, since removed) when the
-- stack moved off XAMPP/MySQL to Supabase Postgres.
--
-- Conventions:
